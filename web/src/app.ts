...
const go = new Go();
WebAssembly.instantiateStreaming(fetch("/dist/main.wasm"), go.importObject).then(
  result => {
...
const path = window.location.href.split("/").pop() as string;
// let paramString = window.location.href.split('?')[1];
// let queryString = new URLSearchParams(paramString);
const read10kbfile = (path: string): void => {
  fetch("/api/" + path , {
    method: "GET",
  })
    .then(async resp => { 
      firstInput = true;
      startSession(await resp.text())
      // return resp.text()
    })
}
